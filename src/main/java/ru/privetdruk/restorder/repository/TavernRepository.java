...
public interface TavernRepository extends CrudRepository<TavernEntity, Long> {
    ...
    @Query("SELECT tavern FROM TavernEntity tavern WHERE tavern.id = :id")
    TavernEntity findByIdWithoutAllData(Long id);
    ...
    TavernEntity findByNameAndCategory(String name, Category category);
}
